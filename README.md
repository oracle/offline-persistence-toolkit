...
```javascript
  requirejs.config({
    paths: {
      'persist' : 'js/libs/persist/v1.1.9/min'

      // Other path mappings here
 }
```
For Oracle JET apps, also open `appDir/src/js/main-release-paths.json` and add the `'persist' : 'js/libs/persist/v1.1.9/min'` entry to the list of paths.
...
It is recommended to add the version number as a convention in your application build step such as `'persist' : 'js/libs/persist/v1.1.9/min'`.
