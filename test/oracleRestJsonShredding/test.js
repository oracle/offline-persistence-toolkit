  ...
  function (oracleRestJsonShredding, logger) {
    ...
    asyncTest('getShredder()', function (assert) {
      ...
      shredder(response).then(function (shreddedData) {
        ...
        ok(JSON.stringify(data) == shreddedDataJsonString, 'shreddedData item data is correct');
    ...
    asyncTest('getShredder() with complex key', function (assert) {
      expect(15);
      var data = [{DepartmentId: 1001, DepartmentName: 'ADFPM 1001 neverending', LocationId: 200, ManagerId: 300},
        {DepartmentId: 556, DepartmentName: 'BB', LocationId: 200, ManagerId: 300},
        {DepartmentId: 10, DepartmentName: 'Administration', LocationId: 200, ManagerId: 300}];
      ...
      shredder(response).then(function (shreddedData) {
        ...
        var shreddedDataJsonString = JSON.stringify(shreddedData[0].data);
        ok(JSON.stringify(data) == shreddedDataJsonString, 'shreddedData item data is correct');
        ok(JSON.stringify(shreddedData[0].keys[0]) == JSON.stringify([1001, 200]), 'shreddedData item keys[0] is correct');
        ok(JSON.stringify(shreddedData[0].keys[1]) == JSON.stringify([556, 200]), 'shreddedData item keys[0] is correct');
        ok(JSON.stringify(shreddedData[0].keys[2]) == JSON.stringify([10, 200]), 'shreddedData item keys[0] is correct');
        shredder(singleResponse).then(function (shreddedData) {
          ok(shreddedData.length == 1, 'shreddedData contains one item');
          ok(shreddedData[0].name == 'departments', 'shreddedData item storename is departments');
          ok(shreddedData[0].data.length == 1, 'shreddedData item data length is 1');
    ...
    asyncTest('getShredder() with complex key with null', function (assert) {
      expect(15);
      ...
      shredder(response).then(function (shreddedData) {
        ...
        var shreddedDataJsonString = JSON.stringify(shreddedData[0].data);
        ok(JSON.stringify(data) == shreddedDataJsonString, 'shreddedData item data is correct');
        ok(JSON.stringify(shreddedData[0].keys[0]) == JSON.stringify([1001, null]), 'shreddedData item keys[0] is correct');
        ok(JSON.stringify(shreddedData[0].keys[1]) == JSON.stringify([556, null]), 'shreddedData item keys[0] is correct');
        ok(JSON.stringify(shreddedData[0].keys[2]) == JSON.stringify([10, null]), 'shreddedData item keys[0] is correct');
        shredder(singleResponse).then(function (shreddedData) {
          ok(shreddedData.length == 1, 'shreddedData contains one item');
          ok(shreddedData[0].name == 'departments', 'shreddedData item storename is departments');
          ok(shreddedData[0].data.length == 1, 'shreddedData item data length is 1');
    ...
    asyncTest('getUnshredder()', function (assert) {
      ...
      unshredder(unshreddedValue, new Response(null, {
        ...
        response.json().then(function (updatedPayload) {
          ok(JSON.stringify(payload) == JSON.stringify(updatedPayload.items), 'unshredded payload is correct');
